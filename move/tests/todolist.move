...
// //     test_scenario::next_tx(&mut scenario, owner);
// //     {
// //         let todolist_val = test_scenario::take_from_sender<TodoList>(&scenario);
// //         let ctx = test_scenario::ctx(&mut scenario);
        
// //         // Task 추가
// //         let content = string::utf8(b"Toggle Test Task");
// //         todolist::add_task(&mut todolist_val, content, ctx);
        
// //         // 초기 상태 확인 - completed가 false인지
// //         let tasks = todolist::get_value(&todolist_val);
// //         let task = vector::borrow(&tasks, 0);
...
// //         // 토글 후 상태 확인 - completed가 true로 변경되었는지
// //         let tasks = todolist::get_value(&todolist_val);
// //         let task = vector::borrow(&tasks, 0);
